...
//----------------------------------------------------------------------------------------------
//prac5_3
//Declaracion del identificador del mutex y el nombre como variable global
rtems_id MutexId;
...
)
{
	...
	rtems_semaphore_create(MutexName, 1, mutex_attribute, RTEMS_NO_PRIORITY, &MutexId);
				//Es un semaforo de tamaño 1, un cerrojo.
	//---------------------------------------------------------------------
	...
	rtems_task ROBOTCalculatePath (rtems_task_argument unused){		//Se ejecuta cada 4 ticks, con offset 1
		...
		while(1){
			// ----- Prac5_3 ----- Obtenemos el recurso compartido y lo bloqueamos -----
			rtems_semaphore_obtain(MutexId, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
