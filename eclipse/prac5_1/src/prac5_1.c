...
)
{
	...
	rtems_task ROBOTAvoidObstacles (rtems_task_argument unused){	//Se ejecuta cada 2 ticks
		//TODO Completar
		while(1){										//Este bucle consigue printear (usar el)
			leon3_print_string("Avoid Obstacles \n");	//recurso y despues dormirse el tiempo
			status = rtems_task_wake_after(2);			//correspondiente, así infinitas iteraciones
		}
	}
	...
	rtems_task ROBOTAdvanceInPath (rtems_task_argument unused){	//Se ejecuta cada 4 ticks, con offset 3
		...
		//TODO Completar
		while(1){
			printf("Ciclo completado: rtems_ticks_since_boot -> (+4) %i\n", get_ticks_since_boot());
														//Este bucle consigue printear (usar el)
			leon3_print_string("Advance \n");			//recurso y despues dormirse el tiempo
			status = rtems_task_wake_after(4);			//correspondiente, así infinitas iteraciones
		}
	...
	// --------------------------- Prac5_1 ---------------------------
	status = rtems_task_start( Task_id[ 1 ], ROBOTAvoidObstacles, 1 );	// Mandamos a ejecutar las
	status = rtems_task_start( Task_id[ 2 ], ROBOTAdvanceInPath, 2 );	// 3 funciones creadas
	status = rtems_task_start( Task_id[ 3 ], ROBOTCalculatePath, 3 );	// anteriormente.
